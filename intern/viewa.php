...
/*if(!isset($_SESSION['login_user'])) 
    ...
    if(isset($_GET['kit']) && isset($_GET['k'])){
          ...
          $ticketno = $_GET['k'];
    $result = $db->query("SELECT sno,priority,location,name,site,technician,type,category,subcategory,item,description,des,ticketno,status,mode,email,subject,resolution,reason,file,time1 FROM change1 where ticketno = '$ticketno' ");}


  if(isset($_GET['kit']) && isset($_GET['k1'])){
           $technician = $_GET['k1'];
    $result = $db->query("SELECT sno,priority,location,name,site,technician,type,category,subcategory,item,description,des,ticketno,status,mode,email,subject,resolution,reason,file,time1 FROM change1 where technician = '$technician' ");}



    if(isset($_GET['kit']) && isset($_GET['k2'])){
           $category= $_GET['k2'];
    $result = $db->query("SELECT sno,priority,location,name,site,technician,type,category,subcategory,item,description,des,ticketno,status,mode,email,subject,resolution,reason,file,time1 FROM change1 where category = '$category' ");}
    ...
    if(isset($_GET['kit'])  && isset($_GET['k1'])  && isset($_GET['k2']) ){
           $technician = $_GET['k1'];
            $category= $_GET['k2'];
    $result = $db->query("SELECT sno,priority,location,name,site,technician,type,category,subcategory,item,description,des,ticketno,status,mode,email,subject,resolution,reason,file,time1 FROM change1 where technician = '$technician' and category = '$category' ");}
   ...
   $todate1 = $_GET['k4'];
   $status1 = $_GET['k5'];
    $result = $db->query("SELECT sno,priority,location,name,site,technician,type,category,subcategory,item,description,des,ticketno,status,mode,email,subject,resolution,reason,file,time1 FROM change1 where ticketno in (SELECT ticketno from status where status='$status' and date1 between '$fromdate' and '$todate') ");
...
$ip="";
       ...
       // echo "<td><a name='$sno' href=img.php>Click Here</a></td>";
        ...
        echo "<td><form method='GET' action='edita.php' ><button name='o' class='but' value='$sno'>Edit</button></form></td>";
        echo "<td><form method='GET' action='pdf.php' ><button name='generate_pdf' class='but' value='$sno'>Print</button></form></td>";
