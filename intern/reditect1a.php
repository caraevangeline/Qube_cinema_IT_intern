...
if($technician1=="")
{
  ...
  $query = "UPDATE incident SET description = '$d',priority= '$priority1',status= '$status1' WHERE sno='$sno'";
}
else

$query = "UPDATE incident SET technician = '$technician1', description='$d',priority= '$priority1',status= '$status1' WHERE sno='$sno'";


if(mysqli_query($link,$query)) 
{ echo "<script type='text/javascript'>alert('Changes are saved successfully')</script>";

     echo "<script>setTimeout(\"location.href = 'database3a.php';\",1500);</script>";
}
